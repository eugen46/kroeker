...
function footer(lang) {
  const t = T[lang];
  return `<footer class="footer"><div class="footer-main">${esc(UI[lang].footer)}</div><div class="footer-links"><a id="footer-privacy-link" href="${relPage(lang, "datenschutz")}">${UI[lang].privacy}</a><a href="${relPage(lang, "impressum")}">${UI[lang].impressum}</a><button id="footer-cookie-settings" type="button" onclick="openCookieSettings()">${UI[lang].cookies}</button></div><div class="footer-copy">${t.copyright || ""}</div></footer>`;
}
